...
void check_division(mpf_t num1, mpf_t num2, int *result1, int *result2) {
  mpf_t rop;
  mpf_t rop_helper_float;
  // mpz_t rop_helper_int;
  mpz_t s21_rop;
  mpf_t s21_final;
  mpf_init(s21_final);
  mpf_init(rop);
  mpf_init(rop_helper_float);
  // mpz_init(rop_helper_int);
  mpz_init(s21_rop);
  int ret_value;
  ...
  if (ret_value) {
    ...
    mpf_set_prec(rop, 0);
    mpf_mul(rop_helper_float, num2, rop);
    mpf_sub(rop_helper_float, num1, rop_helper_float);
    gmp_printf("\nmpf res is %.28Ff\n", rop_helper_float);
    convert_decimal_to_mpf(check_helper.bits, s21_rop, s21_final);
    compare(rop_helper_float, s21_final);
  }
  mpf_clear(rop);
  mpf_clear(rop_helper_float);
  // mpz_clear(rop_helper_int);
  mpz_clear(s21_rop);
  mpf_clear(s21_final);
}
